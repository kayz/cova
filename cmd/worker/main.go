...
func main() {
	...
	flag.DurationVar(&stageDelay, "mock-stage-delay", 200*time.Millisecond, "mock processing delay per stage")
	...
	flag.StringVar(&jobStatePath, "job-state-path", "docs/log/jobs_state.json", "path to persisted job state, empty to keep in-memory only")
	flag.StringVar(&metricsPath, "metrics-path", "/metrics", "worker metrics endpoint path")
	flag.StringVar(&healthPath, "health-path", "/healthz", "worker health endpoint path")
	flag.Parse()
	...
	if jobStatePath != "" {
		...
		if err != nil {
			log.Fatalf("init job state store: %v", err)
		}
		jobStore = store
		log.Printf("job state will be persisted to %s", jobStatePath)
	}

	handler := mock.NewServer(
		snapshot,
		mock.WithProcessDelay(stageDelay),
		mock.WithWebhookDispatcher(dispatcher),
		mock.WithJobStore(jobStore),
		mock.WithWebhookDeliveryReader(deliveryReader),
	)
	...
	log.Printf("mock worker listening on %s, enabled experts=%d, registry=%s (metrics=%s health=%s)", addr, len(snapshot.Experts), registryPath, metricsPath, healthPath)
